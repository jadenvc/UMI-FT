...
def load_episode_data(episode_folder):
    ...
    if os.path.exists(policy_inference_path):
        ...
        episode_data["ts_targets"] = policy_data["ts_targets_0"][:].reshape(-1, 7)
...
def visualize_episode(episode_folder):
    ...
    if "robot_data" in data and "ts_targets" in data and "timestamps" in data:
        plot_policy_vs_robot_data(
            data["robot_data"], data["ts_targets"], data["timestamps"], output_dir="."
        )
...
if __name__ == "__main__":
    ...
    episode_folders = [
        os.path.join(dataset_folder, ep)
        for ep in sorted(os.listdir(dataset_folder))[:1]
    ]
