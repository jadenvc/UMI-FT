...
def process_one_episode_into_zarr(episode_name, output_root, config):
    ...
    print(
        f"[process_one_episode_into_zarr] episode_name: {episode_name}, episode_id: {episode_id}"
    )
    ...
    try:
        for id in config["id_list"]:
            ...
            if os.path.exists(json_path):
                ...
                data_wrench_time_stamps.append(
                    df_wrench_data["wrench_time_stamps"].to_numpy()
                )
                data_wrench.append(np.vstack(df_wrench_data["wrench"]))
                data_wrench_filtered.append(
                    np.vstack(df_wrench_data["wrench_filtered"])
                )
            ...
            if os.path.exists(json_path):
                ...
                data_gripper_time_stamps.append(
                    df_gripper_data["eoat_time_stamps"].to_numpy()
                )
            ...
            if config["ft_sensor_configuration"] == "handle_on_sensor":
                ...
                robot_wrench_id = np.searchsorted(
                    data_robot_time_stamps[id], data_wrench_time_stamps[id]
                )
                ...
                wrench_filtered_net = (
                    data_robot_wrench[id][robot_wrench_id] - data_wrench_filtered[id]
                )
            ...
            if os.path.exists(json_path):
                ...
                data_key_time_stamps.append(
                    df_key_data["key_event_time_stamps"].to_numpy()
                )
        ...
        if config["has_correction"]:
            ...
            if os.path.exists(policy_inference_path):
                ...
                for id in config["id_list"]:
                    data_policy_ts_pose_command.append(
                        policy_data[f"ts_targets_{id}"][:].reshape(-1, 7)
                    )
                    data_policy_time_stamps.append(
                        policy_data["timestamps_s"][:].reshape(-1) * 1000.0
                    )
                    if f"ts_gripper_{id}" in policy_data:
                        data_policy_gripper_command.append(
                            policy_data[f"ts_grippers_{id}"][:].reshape(-1, 1)
                        )
    ...
    recoder_buffer.create_zarr_groups_for_episode(
        rgb_data_shapes, config["id_list"], episode_id
    )
    ...
    for key in root["data"].keys():
        ...
        for id in config["id_list"]:
            ...
            if f"ts_pose_fb_{id}" in ep_data:
                ...
                assert (
                    f"js_fb_{id}" in ep_data
                ), f"Neither ts_pose_fb_{id} nor js_fb_{id} found in episode {episode}"
            ...
            if config["has_correction"]:
                episode_policy_len[id].append(
                    ep_data[f"policy_pose_command_{id}"].shape[0]
                )
...
def compute_vt_for_episode(ep, ep_data, config):
    ...
    for id in config["id_list"]:
        ...
        if not config["flag_real"]:  # for simulation data
            _ft_sensor_pose_fb = ep_data["ft_sensor_pose_fb"]
