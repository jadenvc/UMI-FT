...
def draw_timed_traj_and_rgb(
            ...
            for key, value in additional_data.items():
                ...
                for start, end in zip(
                    ...
                    end_id = np.searchsorted(
                        additional_data_time_stamps[key], end, side="left"
                    )
                    flag_is_correction[start_id:end_id] = True
    ...
    def update_plot():
        ...
        for key, value in data.items():
            ...
            ax_3d.plot(
                value[:, 0], value[:, 1], value[:, 2], alpha=0.6, label=key, color=color
            )
        ...
        if additional_data is not None:
            ...
            for key, value in additional_data.items():
                # plot the whole trajectory
                ax_2d.plot(
                    additional_data_time_stamps[key],
                    value[:, 0],
                    alpha=0.7,
                    label=key + " X",
                )
                ...
                # find the closest data point to the current
                closest_data_idx = find_closest_index(
                    additional_data_time_stamps[key], current_time
                )
                closest_data = value[closest_data_idx]
                title = (
                    title
                    + key
                    + f": {closest_data[0]:.2f}, {closest_data[1]:.2f}, {closest_data[2]:.2f}\n"
                )
                ax_2d.scatter(
                    current_time, closest_data[0], s=50, label="Current " + key + " X"
                )
                ax_2d.scatter(
                    current_time, closest_data[1], s=50, label="Current " + key + " Y"
                )
                ax_2d.scatter(
                    current_time, closest_data[2], s=50, label="Current " + key + " Z"
                )
