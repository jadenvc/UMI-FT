...
def dist_quats(quat_a, quat_b):
    ...
    assert (
        quat_b.shape[-1] == 4
    ), f"quat_b should be (..., 4) or (4,), got {quat_b.shape}"
    ...
    cos_value = np.clip(
        2 * dot_product * dot_product - 1, -1.0, 1.0
...
def dist_n_pose7(pose7_a, pose7_b, rot_weight):
    ...
    assert (
        pose7_a.shape[-2] == pose7_b.shape[-2]
    ), f"pose7_a and pose7_b should have the same n, got {pose7_a.shape} and {pose7_b.shape}"

    if pose7_a.ndim > 2:
        batch_dims = pose7_a.shape[:-2]  # batch dimensions
        pose7_b = np.broadcast_to(
            pose7_b, batch_dims + pose7_b.shape
        )  # broadcast pose7_b to match batch dimensions
...
def SE3_interp(SE3_a, SE3_b, alpha):
    ...
    slerp = scipy_transform.Slerp(
        [0, 1], scipy_transform.Rotation.concatenate([R_a_scipy, R_b_scipy])
    )
...
def pose7_interp(pose7_a, pose7_b, alpha):
    ...
    slerp = scipy_transform.Slerp(
        [0, 1], scipy_transform.Rotation.concatenate([quat_a_scipy, quat_b_scipy])
    )
...
def test():
    ...
    pose7_a = np.array(
        [[1, 0, 0, 1, 0, 0, 0], [0, 2, 0, 0, 1, 0, 0], [0, 0, 3, 0, 0, 1, 0]]
    )
    print("Pose7 Distance:", dist_pose7(pose7_a, pose7_b, rot_weight=1.0))
