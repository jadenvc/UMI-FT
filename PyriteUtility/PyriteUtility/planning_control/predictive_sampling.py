...
class PredictiveSampling:
    ...
    def noise_sample(self, iter):
        ...
        delta_action = np.clip(
            delta_action,
            self.config["action_bound_lower"],
            self.config["action_bound_upper"],
        )
    ...
    def solve(
        ...
        while iter <= self.config["n_diffusion_steps"]:
            ...
            # logs
            print(
                f"[Predictive sampling] Iter {iter}/{self.config['n_diffusion_steps']}, Cost mean: {cost_mean}, best cost: {np.min(cost)}"
            )
