...
class AdmittanceController:
    ...
    def setForceControlledAxis(self, Tr_new, n_af):
        ...
        self.spt_TTadj_new = (
            np.eye(6) - np.linalg.pinv(self.m_anni) @ self.m_anni
        ) @ self.spt_TTadj
        # Tref doesnt change, so we need to compute the new TrefTadj to get the new Tadj
        self.SE3_TrefTadj = (
            su.SE3_inv(self.SE3_WTref) @ self.SE3_WT @ su.twc_to_SE3(self.spt_TTadj_new)
        )
        ...
        self.wrench_T_Err_I = (
            self.Tr_inv @ self.diag_force_selection @ self.Tr @ self.wrench_T_Err_I
        )
        self.wrench_T_Err_prev = (
            self.Tr_inv @ self.diag_force_selection @ self.Tr @ self.wrench_T_Err_prev
        )
    ...
    def step(self):
        ...
        # Wrench updates
        self.wrench_T_spring = (
            self.Jac_bodyV_twist @ self.param_stiffness_mat6 @ self.spt_TTadj
        )
        ...
        np.clip(
            self.wrench_T_Err_I,
            -self.param_force_limit,
            self.param_force_limit,
            out=self.wrench_T_Err_I,
        )

        self.wrench_T_PID[:3] = (
            self.param_direct_force_control_P_trans * self.wrench_T_Err[:3]
            + self.param_direct_force_control_I_trans * self.wrench_T_Err_I[:3]
            + self.param_direct_force_control_D_trans
            * (self.wrench_T_Err[:3] - self.wrench_T_Err_prev[:3])
        )
        self.wrench_T_PID[3:] = (
            self.param_direct_force_control_P_rot * self.wrench_T_Err[3:]
            + self.param_direct_force_control_I_rot * self.wrench_T_Err_I[3:]
            + self.param_direct_force_control_D_rot
            * (self.wrench_T_Err[3:] - self.wrench_T_Err_prev[3:])
        )
        ...
        # Force damping
        self.wrench_Tr_damping = (
            -self.Tr @ self.param_damping_mat6 @ self.v_body_ref_last
        )
        self.wrench_Tr_All = self.diag_force_selection @ (
            self.wrench_Tr_spring
            + self.wrench_Tr_Err
            + self.wrench_Tr_PID
            + self.wrench_Tr_damping
        )
        ...
        #      TW=TMTinv Tvd
        #      W_Tr = TMTinv vd_Tr
        self.vd_Tr = np.linalg.solve(
            self.Tr @ self.param_inertia_mat6 @ self.Tr_inv, self.wrench_Tr_All
        )
        ...
        self.v_Tr_f = (
            self.diag_force_selection @ self.v_Tr
        )  # delete velocity in the velocity-controlled direction
        ...
        self.v_body_ref_adj = (
            self.Jac_twist_bodyV @ self.spt_TTadj / self.param_dt
        )  # reference velocity, derived from reference pose
        ...
        # ----------------------------------------
        self.SE3_WT_cmd = (
            self.SE3_WT + su.wedge6(self.v_spatial_cmd) @ self.SE3_WT * self.param_dt
        )
    ...
    def plot_logs(self):
        ...
        fig.add_trace(
            go.Scatter(x=time_s, y=pose_WT[:, 0], name="pose fb", legendgroup="group1"),
            row=1,
            col=1,
        )
        fig.add_trace(
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s, y=pose_WTref[:, 0], name="pose ref", legendgroup="group2"
            ),
            row=1,
            col=1,
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s, y=pose_WTadj[:, 0], name="pose adjusted", legendgroup="group3"
            ),
            row=1,
            col=1,
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s, y=pose_WT_cmd[:, 0], name="pose cmd", legendgroup="group4"
            ),
            row=1,
            col=1,
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["v_body_ref_adj"][:, 0],
                name="vel body ref adjusted",
                legendgroup="group6",
            ),
            row=1,
            col=2,
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["v_spatial_cmd"][:, 0],
                name="vel spatial output",
                legendgroup="group7",
            ),
            row=1,
            col=2,
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["v_Tr_f"][:, 0],
                name="Tr vel output f",
                legendgroup="group9",
            ),
            row=1,
            col=3,
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["v_Tr_v"][:, 0],
                name="Tr vel output v",
                legendgroup="group10",
            ),
            row=1,
            col=3,
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["wrench_T_Err_prev"][:, 0],
                name="wrench T Err prev",
                legendgroup="group15",
            ),
            row=1,
            col=4,
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["wrench_T_Err_prev"][:, 1],
                name="wrench T Err prev",
                legendgroup="group15",
                showlegend=False,
            ),
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["wrench_T_Err_prev"][:, 2],
                name="wrench T Err prev",
                legendgroup="group15",
                showlegend=False,
            ),
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["wrench_T_Err_prev"][:, 3],
                name="wrench T Err prev",
                legendgroup="group15",
                showlegend=False,
            ),
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["wrench_T_Err_prev"][:, 4],
                name="wrench T Err prev",
                legendgroup="group15",
                showlegend=False,
            ),
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["wrench_T_Err_prev"][:, 5],
                name="wrench T Err prev",
                legendgroup="group15",
                showlegend=False,
            ),
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["wrench_T_spring"][:, 0],
                name="wrench T spring",
                legendgroup="group18",
            ),
            row=1,
            col=4,
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["wrench_Tr_damping"][:, 0],
                name="wrench Tr damping",
                legendgroup="group23",
            ),
            row=1,
            col=5,
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["wrench_Tr_damping"][:, 1],
                name="wrench Tr damping",
                legendgroup="group23",
                showlegend=False,
            ),
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["wrench_Tr_damping"][:, 2],
                name="wrench Tr damping",
                legendgroup="group23",
                showlegend=False,
            ),
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["wrench_Tr_damping"][:, 3],
                name="wrench Tr damping",
                legendgroup="group23",
                showlegend=False,
            ),
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["wrench_Tr_damping"][:, 4],
                name="wrench Tr damping",
                legendgroup="group23",
                showlegend=False,
            ),
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["wrench_Tr_damping"][:, 5],
                name="wrench Tr damping",
                legendgroup="group23",
                showlegend=False,
            ),
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["wrench_Tr_spring"][:, 0],
                name="wrench Tr spring",
                legendgroup="group24",
            ),
            row=1,
            col=5,
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["wrench_Tr_spring"][:, 1],
                name="wrench Tr spring",
                legendgroup="group24",
                showlegend=False,
            ),
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["wrench_Tr_spring"][:, 2],
                name="wrench Tr spring",
                legendgroup="group24",
                showlegend=False,
            ),
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["wrench_Tr_spring"][:, 3],
                name="wrench Tr spring",
                legendgroup="group24",
                showlegend=False,
            ),
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["wrench_Tr_spring"][:, 4],
                name="wrench Tr spring",
                legendgroup="group24",
                showlegend=False,
            ),
        ...
        fig.add_trace(
            go.Scatter(
                x=time_s,
                y=self.logs["wrench_Tr_spring"][:, 5],
                name="wrench Tr spring",
                legendgroup="group24",
                showlegend=False,
            ),
