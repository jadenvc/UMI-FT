...
class ShootingSampling:
    ...
    def rand_vel_sample(self, n_vel_samples):
        ...
        gripper_delta_action = np.random.normal(
            loc=robot_nominal_vel, scale=0.1, size=(n_vel_samples, 1, dim - 4)
        )
        ...
        return np.concatenate(
            [gripper_delta_action, gripper_vel * np.ones((n_vel_samples, 1, 4))], axis=2
        )
    ...
    def solve(
        ...
        (
            states_all,
            actions_all,
            env_safety_num_frames,
            pose7_items_stabilized,
            target_item_ids_all,
        ) = self.batch_simulate_func(
        ...
        pose7_items_stabilized = list(
            pose7_items_stabilized[env_safety_num_frames >= 3]
        )
