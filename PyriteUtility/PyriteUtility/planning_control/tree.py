...
class Tree:
    ...
    def __init__(
        self,
        node_property_schema: Dict[str, type],
    ...
    def add_edge_properties(
        ...
        if child_node not in parent_node.children:
            raise ValueError(
                f"No parent-child relationship between {parent_id} and {child_id}"
            )
    ...
    def get_node_property_by_id(
        self, node_id: int, property_name: str
    ...
    def update_node_property(
        self, node_id: int, property_name: str, value: Union[np.ndarray, str]
    ):
        """
        ...
        elif isinstance(expected_type, str) and not isinstance(value, str):
    ...
    def _grow_arrays(self):
        ...
        print(
            f"Arrays grown to capacity: {new_capacity}"
...
if __name__ == "__main__":
    ...
    # Add edge properties
    tree.add_edge_properties(0, 1, {"weight": np.array([0.8]), "label": "strong"})

    tree.add_edge_properties(0, 2, {"weight": np.array([0.3]), "label": "weak"})
    ...
    print(
        f"Position in concatenated array: {tree.get_node_property_concatenated('position')[1]}"
    )
