...
class BoxGraspSampler:
    ...
    def __init__(self, box_size, pose7_TipHand, config):
        ...
        altitude = (
            np.array(altitude_degrees) * np.pi / 180.0
        )  # 35 degrees and 70 degrees in radians
        longitude = np.linspace(
            0, 2 * np.pi, longitude_num, endpoint=False
        )  # 8 evenly spaced angles around the circle
        ...
        hand_top = hand_axes
        hand_bottom = np.column_stack(
            (hand_axes[:, 0], hand_axes[:, 1], -hand_axes[:, 2])
        )
        hand_left = np.column_stack(
            (hand_axes[:, 0], -hand_axes[:, 2], hand_axes[:, 1])
        )
        hand_right = np.column_stack(
            (hand_axes[:, 0], hand_axes[:, 2], -hand_axes[:, 1])
        )
        hand_front = np.column_stack(
            (hand_axes[:, 2], hand_axes[:, 1], -hand_axes[:, 0])
        )
        hand_back = np.column_stack(
            (-hand_axes[:, 2], hand_axes[:, 1], hand_axes[:, 0])
        )
        ...
        probing_poses_top = np.array(
            [
                np.concatenate((fingertip, hand_axis))
                for fingertip in top_fingertips
                for hand_axis in hand_top
            ]
        )
        probing_poses_bottom = np.array(
            [
                np.concatenate((fingertip, hand_axis))
                for fingertip in bottom_fingertips
                for hand_axis in hand_bottom
            ]
        )
        probing_poses_left = np.array(
            [
                np.concatenate((fingertip, hand_axis))
                for fingertip in left_fingertips
                for hand_axis in hand_left
            ]
        )
        probing_poses_right = np.array(
            [
                np.concatenate((fingertip, hand_axis))
                for fingertip in right_fingertips
                for hand_axis in hand_right
            ]
        )
        probing_poses_front = np.array(
            [
                np.concatenate((fingertip, hand_axis))
                for fingertip in front_fingertips
                for hand_axis in hand_front
            ]
        )
        probing_poses_back = np.array(
            [
                np.concatenate((fingertip, hand_axis))
                for fingertip in back_fingertips
                for hand_axis in hand_back
            ]
        )
        ...
        self.tip_probing_poses = np.concatenate(
            (
                self.tip_probing_poses,
                np.full((self.tip_probing_poses.shape[0], 1), 0.0),
            ),
            axis=1,
        ...
        x_range = np.linspace(
            -lx + grasp_side_edge_buffer, lx - grasp_side_edge_buffer, nx
        )
        y_range = np.linspace(
            -ly + grasp_side_edge_buffer, ly - grasp_side_edge_buffer, ny
        )
        z_range = np.linspace(
            -lz + grasp_side_edge_buffer, lz - grasp_side_edge_buffer, nz
        )
        ...
        top1_grasp_poses = np.array(
            [
                np.concatenate((fingertip, top1_quat, [top1_width]))
                for fingertip in top_fingertips1
            ]
        )
        top2_grasp_poses = np.array(
            [
                np.concatenate((fingertip, top2_quat, [top2_width]))
                for fingertip in top_fingertips2
            ]
        )
        bottom1_grasp_poses = np.array(
            [
                np.concatenate((fingertip, bottom1_quat, [bottom1_width]))
                for fingertip in bottom_fingertips1
            ]
        )
        bottom2_grasp_poses = np.array(
            [
                np.concatenate((fingertip, bottom2_quat, [bottom2_width]))
                for fingertip in bottom_fingertips2
            ]
        )
        left1_grasp_poses = np.array(
            [
                np.concatenate((fingertip, left1_quat, [left1_width]))
                for fingertip in left_fingertips1
            ]
        )
        left2_grasp_poses = np.array(
            [
                np.concatenate((fingertip, left2_quat, [left2_width]))
                for fingertip in left_fingertips2
            ]
        )
        right1_grasp_poses = np.array(
            [
                np.concatenate((fingertip, right1_quat, [right1_width]))
                for fingertip in right_fingertips1
            ]
        )
        right2_grasp_poses = np.array(
            [
                np.concatenate((fingertip, right2_quat, [right2_width]))
                for fingertip in right_fingertips2
            ]
        )
        front1_grasp_poses = np.array(
            [
                np.concatenate((fingertip, front1_quat, [front1_width]))
                for fingertip in front_fingertips1
            ]
        )
        front2_grasp_poses = np.array(
            [
                np.concatenate((fingertip, front2_quat, [front2_width]))
                for fingertip in front_fingertips2
            ]
        )
        back1_grasp_poses = np.array(
            [
                np.concatenate((fingertip, back1_quat, [back1_width]))
                for fingertip in back_fingertips1
            ]
        )
        back2_grasp_poses = np.array(
            [
                np.concatenate((fingertip, back2_quat, [back2_width]))
                for fingertip in back_fingertips2
            ]
        )
        ...
        # print out summary
        print(
            f"BoxGraspSampler initialized with {self.tip_probing_poses.shape[0]} probing poses and {self.grasp_poses_in_item_frame.shape[0]} grasp poses."
        )
    ...
    def sample_hand_poses(self, SO3_nominal_hand):
        ...
        """
        assert (
            SO3_nominal_hand.shape == (3, 3)
        ), f"SO3_nominal_hand should be of shape (3, 3), but got {SO3_nominal_hand.shape}"

        # Filter probing poses
        vecs_Y = -self.tip_probing_poses[:, 3:6]
        vec_Y_nominal_gripper = SO3_nominal_hand[
            :, 1
        ]  # -Y is assumed to be the gripper axis (finger to palm)
        vec_Y_nominal_gripper = vec_Y_nominal_gripper / np.linalg.norm(
            vec_Y_nominal_gripper
        )
        dot = vecs_Y @ vec_Y_nominal_gripper
        angles = np.arccos(np.clip(dot, -1.0, 1.0))
        valid_probing_poses = self.tip_probing_poses[
            angles < self.robot_tilting_tol_rad
        ]
        ...
        if valid_probing_poses.shape[0] > 0:
            ...
            probing_SO3 = su.transpose(
                np.stack((probing_X, probing_Y, probing_Z), axis=1)
            )
            ...
            pose7_ITip_probing = np.concatenate(
                (valid_probing_poses[:, :3], probing_quats), axis=1
            )
            SE3_ITip_probing = su.pose7_to_SE3(
                pose7_ITip_probing
            )  # Convert probing poses to SE3
            SE3_IHand_probing = (
                SE3_ITip_probing @ self.SE3_TipHand
            )  # Convert to hand poses
        ...
        valid_grasp_poses = self.grasp_poses_in_item_frame[
            angles_grasp < self.robot_tilting_tol_rad
        ]

        # symmetry: choose the grasp with the smallest angle to the nominal
        vec_Z_nominal_gripper = SO3_nominal_hand[
            :, 2
        ]  # Z is assumed to be perpendicular to the grasp plane
        vec_Z_nominal_gripper = vec_Z_nominal_gripper / np.linalg.norm(
            vec_Z_nominal_gripper
        )
        R180y = np.array([[-1, 0, 0], [0, 1, 0], [0, 0, -1]])
...
if __name__ == "__main__":
    ...
    pose7_TipHand = np.array(
        [0, -0.222, 0, 1, 0, 0, 0]
    )  # Example pose of the gripper tip in the hand frame
