...
class RRT(Tree):
    ...
    def __init__(
        ...
        # initialize the tree
        super().__init__(
            node_property_schema=node_property_schema,
            edge_property_schema=edge_property_schema,
            initial_capacity=rrt_config["tree_initial_capacity"],
            growth_factor=rrt_config["tree_growth_factor"],
        )

        # initialize the first node of the tree
        self.add_node(node_id=0, properties=initial_node_properties)
    ...
    def sample_random_states(self):
        ...
        if random_number < self.goal_sampling_probability:
            ...
            pose3_all = np.random.uniform(
                self.items_pose3_lower,
                self.items_pose3_upper,
                size=(self.number_of_items, 3),
            )
    ...
    def find_nearest_node(self, target_state):
        ...
        """
        pose7_all_nodes_all_items = self.get_node_property_concatenated(
            "pose7_items"
        )  # (N_nodes, n_items, 7)

        # Calculate distances from all nodes to the target pose
        distances_all = su.dist_pose7(
            pose7_all_nodes_all_items, target_state, rot_weight=self.rot_weight
        )  # (N_nodes, n_items)
        distances_average = np.mean(
            distances_all, axis=1
        )  # Average distance across all items
    ...
    def steer(self, nearest_node_id, pose7_WGoal_all):
        ...
        item_poses_stabilized = nearest_node.properties[
        ...
        for i in range(self.number_of_items):
            dist_to_goal = su.dist_pose7(
                nearest_node.properties["pose7_items"][i],
                pose7_WGoal_all[i],
        ...
        if target_item_id_current in item_not_at_goal_ids:
            ...
            if (
                is_grasp_current is not None
            ):  # if None, it means the first node, no grasp assigned to this node
                (
                    first_states_list,
                    first_actions_list,
                    first_pose7_items_stabilized,
                    first_target_item_ids,
                ) = self.steer_solver_func(
        ...
        for target_item_id in item_not_at_goal_ids:
            feasible_grasp_poses, feasible_probe_poses = (
                self.compute_feasible_hand_configs_func(
                    nearest_node.properties["pose7_items"], target_item_id
                )
            )
            ...
            if (
                num_feasible_grasp_poses + num_feasible_probe_poses
                > self.max_num_steer_samples
            ):
                # try to sample equal amount of grasp poses and probe poses
                if num_feasible_grasp_poses > num_feasible_probe_poses:
                    num_grasp_to_sample = min(
                        num_feasible_grasp_poses, self.max_num_steer_samples // 2
                    )
                    num_probe_to_sample = (
                        self.max_num_steer_samples - num_grasp_to_sample
                    )
                    if num_probe_to_sample > num_feasible_probe_poses:
                        num_probe_to_sample = num_feasible_probe_poses
                        num_grasp_to_sample = (
                            self.max_num_steer_samples - num_probe_to_sample
                        )
                else:
                    num_probe_to_sample = min(
                        num_feasible_probe_poses, self.max_num_steer_samples // 2
                    )
                    num_grasp_to_sample = (
                        self.max_num_steer_samples - num_probe_to_sample
                    )
                ...
                feasible_grasp_poses = feasible_grasp_poses[
                    np.random.choice(
                        feasible_grasp_poses.shape[0],
                        num_grasp_to_sample,
                        replace=False,
                    )
                ]
                feasible_probe_poses = feasible_probe_poses[
                    np.random.choice(
                        feasible_probe_poses.shape[0],
                        num_probe_to_sample,
                        replace=False,
                    )
                ]
            ...
            if num_feasible_grasp_poses == 0:
                ...
                feasible_hand_poses = np.concatenate(
                    (feasible_grasp_poses, feasible_probe_poses), axis=0
                )
                # create a vector to indicate whether a hand pose is a grasp or not
                is_grasp_pose = np.concatenate(
                    (
                        np.ones(feasible_grasp_poses.shape[0]),
                        np.zeros(feasible_probe_poses.shape[0]),
                    ),
                    axis=0,
                )
        ...
        if total_feasible_poses == 0:
            ...
            print(
                f"[RRT.steer] No feasible hand configurations found at node {nearest_node_id}."
            )
            return (
                first_states_list,
                first_actions_list,
        ...
        print(
            f"[RRT.steer] Trying {hand_poses_all.shape[0]} feasible hand poses for item {target_item_ids_all} at node {nearest_node_id}."
        )
        (
            second_states_list,
            second_actions_list,
            second_pose7_items_stabilized,
            second_target_item_ids,
        ) = self.steer_solver_func(
        ...
        all_pose7_items_stabilized = (
            first_pose7_items_stabilized + second_pose7_items_stabilized
        )
    ...
    def plan(self, env, log_folder_path=None):
        ...
        if not self._arrays_initialized:
            raise RuntimeError(
                "[RRT] Tree not initialized. Call set_initial_node() first."
            )
        ...
        while True:
            ...
            if n_iters > self.max_iters:
                print("[RRT] Max iterations reached without finding a path.")
                break

            print(
                f"[RRT] Iteration {n_iters}, New Nodes: {new_nodes}/{self.max_new_nodes}"
            )
            ...
            current_item_poses = self.nodes[nearest_node_id].properties[
                "pose7_items"
            ]  # (n_items, 7)
            distances_to_goal = su.dist_pose7(
                sampled_goal_item_poses, current_item_poses, rot_weight=self.rot_weight
            )  # (n_items,)
            ...
            (
                result_states,
                result_actions,
                result_stabilized_item_poses,
                result_target_item_ids,
            ) = self.steer(nearest_node_id, sampled_goal_item_poses)
            if len(result_states) == 0:
                print(
                    f"[RRT] No valid steering found for target item {target_item_id}."
                )
                continue
            print(
                f"[RRT] Steering solved for node {nearest_node_id}, item {target_item_id}, found {len(result_states)} feasible states."
            )

            for state, action, pose7_items_stabilized, target_item_id in zip(
                result_states,
                result_actions,
                result_stabilized_item_poses,
                result_target_item_ids,
                ...
                assert (
                    len(state.is_grasp) == 1
                ), "Sanity check: Here each state should correspond to a single environment."
                new_node_properties = {
                    ...
                    "joints_current_hand": np.array(
                        state.robot_dof_positions[-1, :]
                    ),  # Last frame
                ...
                new_edge_properties = {
                    ...
                    "robot_dof_positions": np.array(
                        state.robot_dof_positions
                    ),  # Whole traj
                ...
                self.add_edge_properties(
                    nearest_node_id, new_node_id, new_edge_properties
                )
                new_nodes += 1
                ...
                if (
                    np.mean(
                        su.dist_pose7(
                            new_item_poses, self.goal_items_pose7, self.rot_weight
                        )
                    )
                    ...
                    for n in range(Nnodes - 1):
                        ...
                        if len(pose7_items_all) != len(robot_dof_forces):
                            print(
                                f"[RRT.plan] Edge from {start_id} to {end_id} has inconsistent lengths: pose7_items {len(pose7_items_all)}, robot_dof_forces {len(robot_dof_forces)}"
                            )
                            assert False
