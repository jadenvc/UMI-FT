...
class ModelPredictiveController:
    ...
    def compute_one_horizon_action(self, obs_task, device):
        ...
        with torch.no_grad():
            ...
            # add batch dimension
            obs_sample_np = dict_apply(
                obs_sample_np, lambda x: rearrange(x, "... -> 1 ...")
            )
            # convert to torch tensor
            ...
            action = self.action_postprocess(
                raw_action, SE3_WBase, self.id_list, self.fix_orientation
            )
            printOrNot(self.verbose_level, "Sparse inference latency:", time.time() - s)
            return action
