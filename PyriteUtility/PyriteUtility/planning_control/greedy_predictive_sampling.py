...
import numpy as np
from PyriteGenesis.envs.genesis_base_task_env import State
...
class GreedyPredictiveSampling:
    ...
    def __init__(
        self, config, batch_simulate_func, validify_action_func, log_folder_path
    ):
        self.config = config
    ...
    def rand_vel_sample(self, nominal_vel):
        ...
        assert (
            len(nominal_vel.shape) == 1 and nominal_vel.shape[0] == dim
        ), "nominal_vel must be a 1D array of length action_dim"
        ...
        gripper_delta_action = np.random.normal(
            loc=robot_nominal_vel, scale=0.1, size=(N, 1, dim - 4)
        )

        # normalize the delta_action
        norm = np.linalg.norm(gripper_delta_action, axis=2, keepdims=True)
        norm[norm == 0] = 1  # avoid division by zero
        gripper_delta_action = (
            gripper_delta_action / norm * self.config["action_mag_per_step"]
        )

        return np.concatenate(
            [gripper_delta_action, gripper_vel * np.ones((N, 1, 4))], axis=2
        )
    ...
    def solve(
        ...
        pose7_item_initial = pose7_items_current[
            target_item_id, :
        ].copy()  # Initial pose of the target item
        ...
        while iter <= action_horizon:
            ...
            action_sample = self.rand_vel_sample(
                np.squeeze(action_selected)
            )  # (n, 1, d)
            ...
            if cost_selected >= cost_prev - self.config["cost_improvement_threshold"]:
                print(
                    f"[Predictive sampling] Cost did not improve enough at iter {iter}: {cost_prev} -> {cost_selected}. Breaking the loop."
                )
                break
            else:
            ...
            print(
                f"[Predictive sampling] Iter {iter}/{self.config['action_horizon']}, Cost mean: {np.mean(cost)}, best cost: {np.min(cost)}"
            )
            iter += 1
        ...
        # log the data
        states_all = State()
        states_all.pose7_items = np.array(pose7_items_all)[
            np.newaxis, ...
        ]  # add env axis
        states_all.robot_dof_positions = np.array(robot_state_all)[
            np.newaxis, ...
        ]  # add env axis
        states_all.robot_dof_forces = np.array(robot_forces_all)[
            np.newaxis, ...
        ]  # add env axis
