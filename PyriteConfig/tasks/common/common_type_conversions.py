...
def raw_to_obs(
    ...
    for id in shape_meta["id_list"]:
        ...
        if f"item_poses_{id}" in raw_data:
            ...
            episode_data["obs"][f"item_poses{id}_pos"] = item_poses9[..., :3].reshape(
                item_poses9.shape[0], -1
            )
            episode_data["obs"][f"item_poses{id}_rot_axis_angle"] = item_poses9[
                ..., 3:
            ].reshape(item_poses9.shape[0], -1)
            assert episode_data["obs"][f"item_poses{id}_pos"].shape[1] == n_items * 3
            assert (
                episode_data["obs"][f"item_poses{id}_rot_axis_angle"].shape[1]
                == n_items * 6
            )
        ...
        if "policy_robot0_eef_pos" in shape_meta["obs"].keys():
            ...
            policy_indices = (
                np.searchsorted(policy_timestamps, ts_timestamps, side="right") - 1
            )
            ...
            episode_data["obs"][f"policy_robot{id}_eef_rot_axis_angle"] = policy_pose9[
                ..., 3:
            ]
        else:
            assert (
                not raw_policy_timestamp
            ), "transformer without base action is Not yet implemented."
        ...
        if "key_event_0" in raw_data.keys():
            ...
            episode_data["obs"][f"key_event_time_stamps_{id}"] = raw_data[
                f"key_event_time_stamps_{id}"
            ][:]
...
def raw_to_action3(
    ...
    assert episode_data["action"].shape[1] == 3 or episode_data["action"].shape[1] == 6
...
def raw_to_action15(
    ...
    for id in id_list:
        ...
        action.append(np.concatenate([ts_pose9_command, downsampled_wrench], axis=-1))
    # action: trim to the shortest length
    action_len = min(action_lens)
    action = [x[:action_len] for x in action]
...
def raw_to_action16(
    ...
    for id in id_list:
        ...
        indices = np.searchsorted(
            gripper_command_timestamps, ts_pose_timestamps, side="left"
        )
        indices = np.clip(
            indices, 0, len(gripper_command_timestamps) - 1
        )  # Ensure indices are valid
        downsampled_gripper_command = gripper_command[indices]

        action.append(
            np.concatenate(
                [ts_pose9_command, downsampled_wrench, downsampled_gripper_command],
                axis=-1,
            )
        )
    # action: trim to the shortest length
...
def raw_to_action19(
    ...
    for id in id_list:
        ...
        if f"policy_pose_command_{id}" in raw_data.keys():
            ...
            delta_virtual_target = (
                su.SE3_inv(policy_SE3_command) @ ts_SE3_virtual_target
            )
...
def sparse_obs_to_obs_sample(
    ...
    for key, attr in shape_meta["obs"].items():
        ...
        if type == "timestamp" and key in obs_sparse:
            sparse_obs_processed[key] = obs_sparse[key].astype(np.float32)
    ...
    if f"robot{id_list[0]}_eef_pos" in shape_meta["sample"]["obs"]["sparse"]:
        ...
        for id in id_list:
            ...
            sparse_obs_processed[f"robot{id}_eef_rot_axis_angle"] = pose9_relative[
                ..., 3:
            ]
                ...
                policy_SE3_WT = su.pose9_to_SE3(
                    np.concatenate(
                        [
                            ...
                            sparse_obs_processed[
                                f"policy_robot{id}_eef_rot_axis_angle"
                            ],
                        ],
                        axis=-1,
                    )
            ...
            for key, attr in shape_meta["sample"]["obs"]["sparse"].items():
                ...
                if shape_meta["obs"][key]["type"] == "low_dim":
                    ...
                    shape_meta["obs"][key]["type"] == "timestamp" and key in obs_sparse
                ):
