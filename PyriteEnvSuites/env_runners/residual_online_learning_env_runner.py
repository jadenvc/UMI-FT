...
from PyriteML.online_learning.configs.config_v1 import (
    control_para,
    hardware_para,
    online_learning_para,
)
from PyriteUtility.common import GracefulKiller, dict_apply
...
def scale_and_cap_residual_action(
    r, scale=1, translation_limit=0.05, rotation_limit=0.4
):
    """
...
def main():
    ...
    actor_node = Actor(
        ...
        transitions_topic_expire_time_s=online_learning_para[
            "transitions_topic_expire_time_s"
        ],
    )
    ...
    with hydra.initialize(version_base=None, config_path=policy_workspace_config_path):
        residual_cfg = hydra.compose(
    ...
    residual_weights_loaded = (
        False  # only set to true if received weights from learner.
    )
    ...
    if "policy_robot0_eef_pos" in residual_shape_meta["sample"]["obs"]["sparse"]:
        residual_base_action_query_size = (
            residual_shape_meta["sample"]["obs"]["sparse"]["policy_robot0_eef_pos"][
                "horizon"
            ]
            - 1
        ) * residual_shape_meta["sample"]["obs"]["sparse"]["policy_robot0_eef_pos"][
    ...
    residual_action_down_sample_steps = residual_shape_meta["sample"]["action"][
        "sparse"
    ]["down_sample_steps"]
    residual_action_horizon = residual_shape_meta["sample"]["action"]["sparse"][
        "horizon"
    ]
    ...
    while True:
        ...
        env.start_saving_data_for_a_new_episode(
            online_learning_para["data_folder_path"] + "raw"
        )
        ...
        while not killer.kill_now:
            ...
            base_policy_action = {}
            for id in id_list:
                base_policy_action[f"policy_pose_command_{id}"] = su.SE3_to_pose7(
                    action_sparse_target_mats[id].reshape([-1, 4, 4])
                )[
                    : control_para["sparse_execution_horizon"]
                ]  # [base_policy_horizon, 4, 4]
                base_policy_action[f"policy_time_stamps_{id}"] = (
                    obs_raw["robot_time_stamps_0"][-1] + sparse_action_timesteps_s
                )[
                    : control_para["sparse_execution_horizon"]
                ] * 1000.0  # [base_policy_horizon]
                if action_type == "pose9g1":
                    base_policy_action[f"policy_gripper_command_{id}"] = (
                        action_sparse_eoats[
                            id
                        ].reshape([-1, 1])[: control_para["sparse_execution_horizon"]]
                    )

            log_for_plot["base_action_0"].append(
                base_policy_action["policy_pose_command_0"]
            )
            log_for_plot["base_action_time_stamps_0"].append(
                np.array(base_policy_action["policy_time_stamps_0"]) / 1000.0
            )

            # save the base actions
            for id in id_list:
                base_action_targets_all[id].append(
                    base_policy_action[f"policy_pose_command_{id}"]
                )
                if action_type == "pose9g1":
                    base_action_grippers_all[id].append(
                        base_policy_action[f"policy_gripper_command_{id}"]
                    )
            base_action_timestamps_all.append(
                base_policy_action["policy_time_stamps_0"]
            )
            ...
            while time.time() - residual_time_start < execution_duration_s:
                obs_raw = env.get_observation_from_buffer()
                log_for_plot["rgb_0"].append(
                    copy.copy(obs_raw["rgb_0"][-1])
                )  # obs_raw["rgb_0"][-1] is the latest image
                log_for_plot["rgb_time_stamps_0"].append(
                    obs_raw["rgb_time_stamps_0"][-1]
                )
                log_for_plot["ts_pose_fb_0"].append(obs_raw["ts_pose_fb_0"][-1])
                log_for_plot["robot_time_stamps_0"].append(
                    obs_raw["robot_time_stamps_0"][-1]
                )

                # initialize an empty residual action for plotting
                if "pose9wrench6" in residual_action_type:
                    residual_action = np.zeros(
                        (residual_action_horizon, 15 * len(id_list))
                    )
                    for id in id_list:
                    ...
                    residual_action = np.zeros(
                        (residual_action_horizon, 9 * len(id_list))
                    )
                    for id in id_list:
                    ...
                    residual_action = np.zeros(
                        (residual_action_horizon, 16 * len(id_list))
                    )
                    for id in id_list:
                    ...
                    residual_action = np.zeros(
                        (residual_action_horizon, 10 * len(id_list))
                    )
                    for id in id_list:
                ...
                if residual_weights_loaded:
                    ...
                    raw_to_obs(
                        ...
                        obs_task,
                        residual_shape_meta,
                        raw_policy_timestamp=use_transformers,
                    )
                    residual_obs_data = {}
                    # sub sampling
                    for key, attr in residual_shape_meta["sample"]["obs"][
                        "sparse"
                    ].items():
                        ...
                        if len(data) < (horizon - 1) * down_sample_steps + 1:
                            print(
                                f"Error: {key} is too short. len = {len(data)}, expecting = {(horizon - 1) * down_sample_steps + 1}."
                            )
                    ...
                    with torch.no_grad():
                        ...
                        if use_transformers:
                            ...
                            nobs_sample = residual_normalizer.normalize(
                                obs_dict_sparse_without_time
                            )
                            for key in obs_sample:
                                if "time_stamps" in key:
                                    nobs_sample[key] = obs_sample[key]
                            nobs_sample = dict_apply(
                                nobs_sample, lambda x: x.to(device)
                            )
                            ...
                            nresidual_action = residual_policy.predict_actions(
                                nobs_encode,
                                time_encode,
                                (
                                    1,
                                    residual_shape_meta["sample"]["action"]["sparse"][
                                        "horizon"
                                    ],
                                    residual_shape_meta["action"]["shape"][0],
                                ),
                            )
                        else:
                        ...
                        residual_action = (
                            residual_normalizer["action"]
                            .unnormalize(nresidual_action)
                            .squeeze(0)
                            .numpy()
                        )  # [residual_action_horizon, 19]
                else:
                    time.sleep(0.02)
                ...
                if (
                    residual_action_type == "pose9wrench6"
                    or residual_action_type == "pose9wrench6g1"
                ):
                    residual_action_wrench = -residual_action[..., 9:15]

                    twist_RVt = np.zeros_like(residual_action_wrench)
                    twist_RVt[..., 0] = (
                        residual_action_wrench[..., 0]
                        / control_para["translational_stiffness"][0]
                    )
                    twist_RVt[..., 1] = (
                        residual_action_wrench[..., 1]
                        / control_para["translational_stiffness"][1]
                    )
                    twist_RVt[..., 2] = (
                        residual_action_wrench[..., 2]
                        / control_para["translational_stiffness"][2]
                    )
                    twist_RVt[..., 3:6] = (
                        residual_action_wrench[..., 3:6]
                        / control_para["rotational_stiffness"]
                    )
                ...
                for id in id_list:
                    ...
                    policy_indices = np.clip(
                        ...
                        - 1,
                        0,
                        len(base_policy_action[f"policy_time_stamps_{id}"]) - 1,
                    )
                    SE3_WB = action_sparse_target_mats[id][
                        policy_indices
                    ]  # [residual_action_horizon, 4, 4]
                    if residual_action_type == "pose9wrench6g1":
                        gripper_base = action_sparse_eoats[id][
                            policy_indices
                        ]  # [residual_action_horizon, 1]
                    ...
                    if residual_weights_loaded:
                        if control_para["scale_and_cap_residual_action"]:
                            SE3_WR = np.stack(
                                [
                                    SE3_WB[i]
                                    @ scale_and_cap_residual_action(
                                        SE3_BR[i],
                                        scale=control_para[
                                            "residual_action_scale_ratio"
                                        ],
                                    )
                                    for i in range(residual_action_horizon)
                                ]
                ...
                if "pose9wrench6" in residual_action_type:
                    ...
                    log_for_plot["residual_action_wrench_0"].append(
                        residual_action_wrench
                    )
                log_for_plot["residual_action_0_abs"].append(outputs_ts_targets[0])
                log_for_plot["residual_action_time_stamps_0"].append(
                    obs_raw["robot_time_stamps_0"][-1] + residual_action_time_steps_s
                )
                ...
                for id in id_list:
                    dt_rgb = (
                        env.current_hardware_time_s
                        - obs_raw[f"rgb_time_stamps_{id}"][-1]
                    )
                    dt_ts_pose = (
                        env.current_hardware_time_s
                        - obs_raw[f"robot_time_stamps_{id}"][-1]
                    )
                    dt_wrench = (
                        env.current_hardware_time_s
                        - obs_raw[f"wrench_time_stamps_{id}"][-1]
                    )
                    # print(
                    #     f"[Total latency] obs lagging for robot {id}: dt_rgb: {dt_rgb}, dt_ts_pose: {dt_ts_pose}, dt_wrench: {dt_wrench}"
                    # )
                    if (
                        dt_rgb > control_para["delay_tolerance_s"]
                        or dt_ts_pose > control_para["delay_tolerance_s"]
                        or dt_wrench > control_para["delay_tolerance_s"]
                    ):
                        print("-------------------------------------------------------")
                ...
                env.schedule_controls(
                    ...
                    timestamps=(residual_action_time_steps_s + action_start_time_s)
                    * 1000,
                )
        ...
        policy_inference_group = zarr.group(
            store=zarr.DirectoryStore(episode_folder + "/policy_inference.zarr"),
            overwrite=True,
        )
        for id in id_list:
            policy_inference_group.create_dataset(
                f"ts_targets_{id}", data=np.array(base_action_targets_all[id])
            )
            if action_type == "pose9g1":
                policy_inference_group.create_dataset(
                    f"ts_grippers_{id}", data=np.array(base_action_grippers_all[id])
                )
        ...
        policy_inference_group.create_dataset(
            "timestamps_s", data=np.array(base_action_timestamps_all) / 1000.0
        )

        ## Plot the episode
        if not control_para["no_visual_mode"]:
            log_for_plot["rgb_0"] = log_for_plot["rgb_0"]
            log_for_plot["rgb_time_stamps_0"] = np.array(
                log_for_plot["rgb_time_stamps_0"]
            )
            log_for_plot["ts_pose_fb_0"] = np.array(log_for_plot["ts_pose_fb_0"])
            log_for_plot["robot_time_stamps_0"] = np.array(
                log_for_plot["robot_time_stamps_0"]
            )
            log_for_plot["base_action_0"] = np.array(
                log_for_plot["base_action_0"]
            ).reshape([-1, 7])
            log_for_plot["base_action_time_stamps_0"] = np.array(
                log_for_plot["base_action_time_stamps_0"]
            ).reshape([-1])
            log_for_plot["residual_action_0_ref"] = np.array(
                log_for_plot["residual_action_0_ref"]
            ).reshape([-1, 4, 4])
            if "pose9wrench6" in residual_action_type:
                log_for_plot["residual_action_0_vt"] = np.array(
                    log_for_plot["residual_action_0_vt"]
                ).reshape([-1, 4, 4])
                log_for_plot["residual_action_wrench_0"] = np.array(
                    log_for_plot["residual_action_wrench_0"]
                ).reshape([-1, 6])
            log_for_plot["residual_action_0_abs"] = np.array(
                log_for_plot["residual_action_0_abs"]
            ).reshape([-1, 7])
            log_for_plot["residual_action_time_stamps_0"] = np.array(
                log_for_plot["residual_action_time_stamps_0"]
            ).reshape([-1])
            ...
            if "pose9wrench6" in residual_action_type:
                ...
                additional_data_time_stamps = {
                    "residual_action_wrench_0": log_for_plot[
                        "residual_action_time_stamps_0"
                    ],
                ...
                draw_timed_traj_and_rgb(
                    rgb_images=log_for_plot["rgb_0"],
                ...
                if control_para["send_transitions_to_server"]:
                    ...
                    online_data = {
                        "episode_name": episode_folder[
                            -18:
                        ],  # only keep the episode name, such as episode_1742230408
                    }
        ...
        if c == "j":
            input(
                "Once robot is stopped, leave the robot free, Press Enter to run calibration."
            )
