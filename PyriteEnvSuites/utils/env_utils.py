...
def decode_stiffness(
    SE3_TW,
    SE3_ref_poses,
    SE3_vt_poses,
    stiffnesses,
    default_stiffness,
    default_stiffness_rot,
    target_stiffness_override,
):
    ts_targets_nominal = su.SE3_to_pose7(SE3_ref_poses.reshape([-1, 4, 4]))
    ts_targets_virtual = su.SE3_to_pose7(SE3_vt_poses.reshape([-1, 4, 4]))
    ...
    for i in range(ts_targets_virtual.shape[0]):
        ...
        delta_vec = (SE3_TVt[:3, 3] - SE3_Ttarget[:3, 3]).reshape(3)
        ...
        M = np.diag([target_stiffness, default_stiffness, default_stiffness])
        S = np.array([X, Y, Z]).T
