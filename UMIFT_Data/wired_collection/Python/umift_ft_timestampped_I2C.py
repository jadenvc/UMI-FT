...
INITIAL_SAMPLES     = 500            # Number of initial samples to compute offset
IGNORED_SAMPLES     = 10             # Number of initial samples to ignore before computing offset

#!!!!!!
WINDOW              = 10              # Moving average window for force/moment
...
MODEL_PATHS = ['SFT4-2_MLP_5L_norm_L2.onnx', 'SFT7-3_MLP_5L_norm_L2.onnx']  

NORM_PATHS = ['SFT4-2_norm_constants.mat', 'SFT7-3_norm_constants.mat']
norm_consts = []
for path in NORM_PATHS:
    mat = scipy.io.loadmat(path)['norm_const'].ravel()[0]      # MATLAB struct
    norm_consts.append({
        'mu_x':  mat['mu_x'].astype(np.float32),   # (12,)
        'sd_x':  mat['sd_x'].astype(np.float32),
        'mu_y':  mat['mu_y'].astype(np.float32),   # (6,)
        'sd_y':  mat['sd_y'].astype(np.float32)
    })
...
def mainLoopMultiSensors(savingFileName, run_duration, result_dir):
    ...
    while (time.time() - start_local_time) < run_duration:
        ...
        # ---------- z-score normalisation of pads --------------------------
        const = norm_consts[idx]
        x_n = (offset_data.astype(np.float32) - const['mu_x']) / const['sd_x']
        x_n = x_n.reshape(1, 12)                     # ensure 1×12

        # Inference
        session = sessions[idx]
        input_name = session.get_inputs()[0].name
        ft_cal_n = session.run(None, {input_name: x_n})[0].flatten()  # shape (6,)

        # ---------- de-normalise to physical units -------------------------
        ft_cal = (ft_cal_n * const['sd_y'] + const['mu_y']).flatten()              # (6,)
