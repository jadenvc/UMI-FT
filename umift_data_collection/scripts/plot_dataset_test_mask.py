...
json_path = "/local/real/hjchoi92/repo/PyriteML/scripts/mask_related/umift_mask.json"  # Change this to the actual path

with open(json_path, "r") as f:
    mask_data = json.load(f)

mask_pts = np.array(mask_data["umift_mask_training"], dtype=np.int32)  # Extract mask points
mask_resolution = tuple(mask_data["mask_training_resolution"])  # (H, W)
...
image = np.array(buffer["data"][ep]["rgb_0"][0])
image_resolution = (image.shape[0], image.shape[1])
print("Image resolution: ", image_resolution)
print("Mask resolution: ", mask_resolution)
# assert image_resolution == mask_resolution, "Image resolution does not match mask resolution"
...
def update_image(time):
    ...
    image = images[closest_idx].copy()

    mask = np.ones(image.shape, dtype=np.uint8) * 255
    cv2.fillPoly(mask, [mask_pts], (0, 0, 0))
    masked_image = cv2.bitwise_and(image, mask)

    ax_img.imshow(masked_image)
