...
def slice_zarr_into_episodes_gripper_depth(input_zarr_path, output_zarr_path):
    ...
    # Process episodes
    for episode_idx, (rgb_end, ultrawide_end, robot_end, wrench_end) in enumerate(
        zip(episode_rgbcamera0_len, episode_ultrawide0_len, episode_robotcamera0_len, episode_wrenchrobot0_len)
    ):
        # Calculate start indices
        rgb_start = episode_rgbcamera0_len[episode_idx - 1] if episode_idx > 0 else 0
...
def create_zarr_dataset_gripper_depth(data_dict, output_path, cam_id=0, gripper_id=0, expected_H=101, expected_W=135):
    # Configure compressors
    image_compressor = Blosc(cname='zstd', clevel=5, shuffle=Blosc.SHUFFLE)
    numeric_compressor = Blosc(cname='lz4', clevel=5, shuffle=Blosc.SHUFFLE)
    string_compressor = VLenUTF8()
    # HC TODO: think about how to compress depth information
