...
def plot_force(save_path, data, sample_rate, current_time, window_size, channel_idx):
    ...
    f, t, Sxx = signal.spectrogram(
        windowed_data, fs=sr, nperseg=512, noverlap=512 // 4, nfft=1024
    )
...
save_path = (
    f"/local/real/liuzeyi/Pyrite/PyriteML/local/episode_{episode_id}/robot_{robot_id}"
)
for current_time in np.arange(0, duration, 0.5):
    for channel_idx in range(wrench_robot.shape[1]):
        Sxx_min, Sxx_max = plot_force(
            save_path,
            wrench_robot[:, channel_idx],
            sr,
