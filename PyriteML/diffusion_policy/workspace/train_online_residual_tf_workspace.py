...
class TrainOnlineResidualTFWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        self.obs_encoder: TimmObsEncoderWithForce = hydra.utils.instantiate(
            cfg.obs_encoder
        )
        if os.path.exists(cfg.base_policy_ckpt):
            print(f"==> loading pretrained weights from {cfg.base_policy_ckpt}")
            base_policy_ckpt = torch.load(cfg.base_policy_ckpt, weights_only=False)[
                "state_dicts"
            ]["model"]
                ...
                for model_key in base_policy_ckpt.keys():
                    if model_key.startswith(key_prefix):
                        self.obs_encoder.key_model_map[key].load_state_dict(
                            {
                                model_key[len(key_prefix) + 1 :]: base_policy_ckpt[
                                    model_key
                                ]
                            },
                            strict=False,
                        )
                self.obs_encoder.key_model_map[key].requires_grad_(
                    False
                )  # freeze the encoder
    ...
    def run(self, dataset: BaseImageDataset):
        ...
        if (
            dataset.sampler.num_new_episodes > 0
            and dataset.sampling_weights is not None
        ):
            ...
            weighted_random_sampler = WeightedRandomSampler(
                dataset.sampling_weights, num_samples, replacement=True
            )
            dataloader_cfg = self.cfg.dataloader.copy()
            dataloader_cfg.shuffle = False
            train_dataloader = DataLoader(
                dataset, **dataloader_cfg, sampler=weighted_random_sampler
            )
        else:
            ...
            for local_epoch_idx in range(num_epochs):
                ...
                with tqdm.tqdm(
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        nobs_sparse = self.sparse_normalizer.normalize(
                        ...
                        action_sparse = (
                            self.sparse_normalizer["action"]
                            .normalize(batch["action"]["sparse"])
                            .to(device)
                        )

                        # compute loss
                        raw_loss = self.model.compute_loss(
                            sparse_nobs_encode, time_encode, action_sparse
                        )  # TODO: compute loss
                ...
                if (self.epoch % self.cfg.training.sample_every) == 0:
                    with torch.no_grad():
                        ...
                        nobs_sparse = dict_apply(nobs_sparse, lambda x: x.to(device))
                        sparse_nobs_encode, time_encode = self.obs_encoder(nobs_sparse)
                        pred_action = self.model.predict_actions(
                            sparse_nobs_encode, time_encode, gt_action.shape
                        )
                        ...
                        if len(val_dataloader) > 0:
                            ...
                            for key in obs_dict_sparse:
                                if "time_stamps" not in key:
                                    obs_dict_sparse_without_time[key] = obs_dict_sparse[
                                        key
                                    ]
                            nobs_sparse = self.sparse_normalizer.normalize(
                                obs_dict_sparse_without_time
                            ...
                            for key in obs_dict_sparse:
                                if "time_stamps" in key:
                                    nobs_sparse[key] = obs_dict_sparse[key]
                            nobs_sparse = dict_apply(
                                nobs_sparse, lambda x: x.to(device)
                            )
                            sparse_nobs_encode, time_encode = self.obs_encoder(
                                nobs_sparse
                            )
                            pred_action = self.model.predict_actions(
                                sparse_nobs_encode, time_encode, gt_action.shape
                ...
                print(
                    "==> self.cfg.training.checkpoint_every: ",
                    self.cfg.training.checkpoint_every,
                )
                print(
                    "==> self.epoch % self.cfg.training.checkpoint_every: ",
                    self.epoch % self.cfg.training.checkpoint_every,
                )
                if (self.epoch % self.cfg.training.checkpoint_every) == 0:
                    ...
                    if topk_ckpt_path is not None:
                        ...
                        self.save_checkpoint(
                            path=topk_ckpt_path
                        )  # epoch=xxxx-train_loss=xxxx.ckpt
