...
from PyriteUtility.data_pipeline.processing_functions import (
    compute_vt_for_episode,
    generate_meta_for_zarr,
...
class DynamicDataset(BaseImageDataset):
    ...
    def load_initial_dataset(
        self, dataset_path: str, num_of_initial_episodes, num_of_new_episodes=0
    ):
        ## read existing data from file
        processed_dataset_path = dataset_path + "processed"
        print(
            "[DynamicDataset] loading initial zarr data into store from",
        ...
        if "data" in directory_root and "meta" in directory_root:
            print(
                f"[DynamicDataset] data and meta found in store with num of episodes = {len(directory_root['data'])}"
            )
        else:
            print(
                "[DynamicDataset] 'data' and 'meta' not found in store. Are you starting a new dataset?"
            )
            ...
            _episode_rgb0_len = meta_group.create_dataset(
                "episode_rgb0_len",
                shape=(0,),
                dtype=np.int64,
                compressor=None,
        ...
        replay_buffer_dict = self.raw_episodes_conversion(
            replay_buffer_raw, self.shape_meta
        )
    ...
    def raw_episodes_conversion(
        ...
        for ep in replay_buffer_raw["data"].keys():
            ...
            self.raw_episode_conversion(
                replay_buffer_raw, replay_buffer, ep, shape_meta
            )
    ...
    def get_normalizer(self, **kwargs) -> tuple:
        ...
        for batch in tqdm(dataloader, desc="iterating dataset to get normalization"):
            ...
            for key in self.shape_meta["sample"]["obs"]["sparse"].keys():
                if (
                    self.shape_meta["obs"][key]["type"] == "low_dim"
                    or self.shape_meta["obs"][key]["type"] == "timestamp"
                ):
    ...
    def load_new_episode(self, episode_name: str, ft_sensor_configuration: str):
        ...
        compute_vt_for_episode(
            episode_name, self.ds_root["data"][episode_name], self.virtual_target_config
        )
        ## Now:
    ...
    def load_new_episodes(self, episode_names: list, ft_sensor_configuration: str):
        ...
        self.val_mask = get_val_mask(
            n_episodes=self.replay_buffer_raw.n_episodes,
            val_ratio=self.val_ratio,
            seed=self.seed,
        )
