...
class StateBasedSequenceSampler:
    ...
    def __init__(
        ...
        for key in episode_keys:
            ...
            ids_to_be_added = episodes_start[episode_count] + np.arange(array_length)
            # Down sample the query indices to make the dataset smaller
            epi_id.extend(ep_id_to_be_added[::sparse_query_frequency_down_sample_steps])
            ids.extend(ids_to_be_added[::sparse_query_frequency_down_sample_steps])

        indices = list(zip(epi_id, ids))
        print(
            f"Total usable episodes: {episode_count + 1}, usable indices: {len(indices)}"
        )
