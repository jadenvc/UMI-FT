...
class Actor:
    def __init__(
        ...
        self.transitions_server = rmq.RMQServer(
            server_name="transitions_server",
            server_endpoint=transitions_server_endpoint,
        )
        self.transitions_server.add_topic(
            transitions_topic, transitions_topic_expire_time_s
        )
    ...
    def receive_network_weights(self, workspace):
        ...
        retrieved_data, timestamp = self.network_weight_client.pop_data(
            topic=self.network_weight_topic, n=-1
        )
