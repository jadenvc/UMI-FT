...
from online_learning.configs.config_v1 import (
    learner_para,
...
def main(residual_ws_cfg: OmegaConf):
    ...
    learner_node = Learner(
        ...
        network_weight_expire_time_s=online_learning_para[
            "network_weight_expire_time_s"
        ],
    )
    ...
    print(
        "Creating residual policy workspace according to config: ",
        online_learning_para["policy_workspace_config_name"],
    )
    ## load previous checkpoint file if exists
    if learner_para["residual_ckpt_path"] is not None and os.path.exists(
        checkpoint_folder_path + learner_para["residual_ckpt_path"]
    ):
        print(
            "--------------------------------------------------------------------------"
        )
        ...
        residual_payload = torch.load(
            open(
                pathlib.Path(
                    checkpoint_folder_path + learner_para["residual_ckpt_path"]
                ).joinpath("checkpoints", "latest.ckpt"),
                "rb",
            ),
            map_location="cpu",
            pickle_module=dill,
        )
        residual_workspace.load_payload(
            residual_payload, exclude_keys=None, include_keys=None
        )
    ...
    dataset.load_initial_dataset(
        online_learning_para["data_folder_path"],
    ...
    while True:
        # train
        if dataset.num_of_episodes() < learner_para[
            "num_episodes_before_first_training"
        ] or (
            residual_workspace.epoch > residual_ws_cfg.training.first_time_num_epochs
            and not got_new_episodes
        ):
            print(
                "[Learner] Waiting for enough episodes to train. Current number of episodes: ",
                dataset.num_of_episodes(),
            )
        else:
