...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            print(
                f"Epoch {epoch + 1}/{num_epochs}, Train Loss: {epoch_train_loss:.6f}, Val Loss: {val_loss:.6f}"
            )
...
def plot_predictions(Y_test, predictions):
    ...
    for i in range(6):
        axes[i].scatter(
            Y_test[:, i].cpu().numpy(), predictions[:, i].cpu().numpy(), alpha=0.5
        )
        axes[i].plot(
...
def main():
    ...
    # Evaluate the model
    test_loss, dim_mse, r2_scores, predictions = evaluate_model(
