...
def plot_trajectory(
    fig,
    axes,
    pos_all: np.ndarray,  # (N_episodes, N_points, 3)
    color_id=0,
    name="",
):
    for dim in range(3):
        ax = axes[dim]
        # Plot the segment
        ax.plot(pos_all[:, :, dim].T, color=colors[color_id], alpha=0.7, linewidth=1)


def plot_range(fig, axes, pos_mean: np.ndarray, pos_std: np.ndarray, color_id=0):
    for dim in range(3):
        ax = axes[dim]
        # Plot the segment
        N = pos_mean.shape[0]
...
for data_id in range(len(dataset_paths)):
    ...
    for ep, ep_data in buffer["data"].items():
        ...
        pos_uniform_sampled_ids = np.searchsorted(
            unit_progress_0, uniform_progress, side="left"
        )
