...
class VideoComparisonTool:
    ...
    def setup_ui(self):
        ...
        # Load video buttons
        tk.Button(
            button_frame, text="Load Video 1", command=lambda: self.load_video(1)
        ).pack(side=tk.LEFT, padx=5)
        tk.Button(
            button_frame, text="Load Video 2", command=lambda: self.load_video(2)
        ).pack(side=tk.LEFT, padx=5)
        ...
        tk.Button(
            button_frame, text="Next Frame (→)", command=lambda: self.next_frame(None)
        ).pack(side=tk.LEFT, padx=5)
        ...
        self.canvas1 = tk.Canvas(
            self.canvas1_frame,
            width=self.display_width1,
            height=self.display_height1,
            bg="black",
        )
        self.canvas1.pack()
        ...
        self.canvas2 = tk.Canvas(
            self.canvas2_frame,
            width=self.display_width2,
            height=self.display_height2,
            bg="black",
        )
        self.canvas2.pack()
        ...
        self.status_bar = tk.Label(
            self.root,
            text="Hover over a video to see pixel values",
            bd=1,
            relief=tk.SUNKEN,
            anchor=tk.W,
        )
    ...
    def update_frame(self):
        ...
        if not ret1 or not ret2:
            messagebox.showinfo(
                "End of Video", "Reached the end of one or both videos."
            )
        ...
        frame1_resized = cv2.resize(
            frame1_rgb, (self.display_width1, self.display_height1)
        )
        frame2_resized = cv2.resize(
            frame2_rgb, (self.display_width2, self.display_height2)
        )
    ...
    def show_pixel_info(self, event, source_canvas):
        ...
        if source_canvas == 1:
            if (
                display_x < 0
                or display_x >= self.display_width1
                or display_y < 0
                or display_y >= self.display_height1
            ):
                return
            ...
            original_x1, original_y1 = self.map_coordinates(
                ...
                display_y,
                self.display_width1,
                self.display_height1,
                self.original_width1,
                self.original_height1,
            )
            ...
            original_x2, original_y2 = self.map_coordinates(
                ...
                self.original_width1,
                self.original_height1,
                self.original_width2,
                self.original_height2,
            )
            ...
            display_x2, display_y2 = self.map_coordinates(
                ...
                self.original_height2,
                self.display_width2,
                self.display_height2,
            )

        else:  # source_canvas == 2
            if (
                display_x < 0
                or display_x >= self.display_width2
                or display_y < 0
                or display_y >= self.display_height2
            ):
                return
        ...
        # map pixel to meter
        cm_per_pixel = 200.0 / 255.0
        cm2 = bgr2 * cm_per_pixel
        # Draw crosshair on the display canvases
        self.draw_crosshair(
            display_x1 if source_canvas == 2 else display_x,
        ...
        self.status_bar.config(
            text=f"RGB: ({original_x1}, {original_y1}) |  Depth: {cm2[2]}"
        )
    ...
    def draw_crosshair(self, display_x1, display_y1, display_x2, display_y2):
        ...
        self.canvas1.create_line(
            0, display_y1, self.display_width1, display_y1, fill="red", tags="crosshair"
        )
        self.canvas1.create_line(
            display_x1,
            0,
        ...
        self.canvas2.create_line(
            0, display_y2, self.display_width2, display_y2, fill="red", tags="crosshair"
        )
        self.canvas2.create_line(
            display_x2,
            0,
