...
dataset_paths.append(
    "/shared_local/data/processed/online_stow_nb_v6_take_over/processed"
)
...
def plot_trajectory_segments(
    fig,
    axes,
    val: np.ndarray,
    val_timestamp: np.ndarray,
    ...
    for i, key_time in enumerate(key_timestamps):
        ...
        if not np.any(time_mask):
            print(
                f"Warning: No data points found for key moment {key_time} "
...
for data_id in range(len(dataset_paths)):
    ...
    for ep, ep_data in buffer["data"].items():
        ...
        if ep_count == len(buffer["data"]):
            ...
            name = None
        plot_trajectory_segments(
            fig,
            axes,
            vel_padded,
            robot_time_stamps_0,
            key_event_timestamps_0,
            color_id=data_id,
